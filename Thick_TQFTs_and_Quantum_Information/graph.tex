...
\subsection{Graphs Encoding Algebraic Expressions}\label{subsec:grphalgexp}
...
\item Consider the vertices with no edges. We will call these edgeless vertices.
Notice that the edgeless vertices are all in $V_1$. For each edgless vertex $u$
in $V_1$, we add a copy of it, say $u'$, to $V_2$ -- it is inserted in the
position after the last vertex in $V_2$ with an edge from a vertex above $u$ in
$V_1$. We then add in the edge $(u, u')$. We continue this process with $V_2$ in
place of $V_1$ and $V_3$ in place of $V_2$ until, there is a copy of each
edgeless vertex in each level with edges connecting them.
...
described in the algorithm in \S\ref{subsec:grphalgexp} so that the vertex set
...
\begin{lem}\label{thm:expiso_unique}
...
up to expression isomorphisms from this point onwards. We will define some
...
vertices and edges sets. It is clear that the edge orderings of the vertices of
...
hence acts as a unit for the disjoint operation. It is easy to see that the
associators and unitors for the coproduct in the category of sets are graph
homomorphisms. In fact, these homomorphisms also preserve the edge orderings and
the ordering of the source vertices -- this is an exercise similar to verifying
that disjoint unions of connections are associative and unital, as we saw in
\S\ref{subsec:connctg}. Thus the associators and unitors are expression
isomorphisms.

We then proceed to define a gluing of expression graphs. For an expression
graph $G$, $S(G)$ and $T(G)$ without any edges are themselves expression graphs.
There are obvious order-preserving inclusions $S(G) \hto G \hot T(G)$.

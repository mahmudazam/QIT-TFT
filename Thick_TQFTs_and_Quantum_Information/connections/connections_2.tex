...
Given any smooth compact manifold $M$ with contractible boundary and a complex
bundle $\pi : E \to M$ with fibre $V \cong \C^n$, there exists a complex bundle
...
with $\partial M$ identified with $\partial M \times \set{1}$. Since
$\partial M$ is contractible, so is $C_0$, such that $E|_{C_0}$ is isomorphic to
the trivial bundle. Therefore, $G_{U, V}|_{C_0}$ is smoothly homotopic to
the constant map $U \cap V \cap C_0 \to \Aut(V) : x \mapsto \id_V$. Let
$H_{U, V} : I \times U \cap V \cap C_0 \to \Aut(V)$ be this homotopy so that
$H_{U, V}(1, -) = G_{U, V}|_{C_0}$ and $H_{U, V}(0, x) = \id_V$ for all
